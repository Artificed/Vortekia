import WaiterNavbar from "@/components/navbars/waiter-navbar";
...
export default function WaiterDashboard() {
  ...
  const { restaurantTransactions: pendingRestaurantTransactions } =
    useGetRestaurantTransactionsByRestaurant(assignedRestaurant?.id ?? "");

  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");

  const filteredTransactions = (pendingRestaurantTransactions || []).filter(
  ...
  const forwardOrderToChef = async (transactionId: string) => {
    try {
      ...
      ToastUtils.success({
        description: "Successfully forwarded order to chef!",
      });
