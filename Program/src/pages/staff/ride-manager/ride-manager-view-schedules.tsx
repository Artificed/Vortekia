...
import { useGetRideStaffSchedules } from "@/hooks/data/use-get-ride-staff-schedules";
...
export default function RideManagerViewSchedules() {
  const { rideStaffSchedules } = useGetRideStaffSchedules();
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [selectedStaffId, setSelectedStaffId] = useState<string>("");

  const filteredStaffSchedules =
    rideStaffSchedules?.filter((staffWithSchedule) => {
      const matchesSearch =
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6 mt-10">
          ...
          >
            ...
            <SelectContent>
              ...
              {rideStaffSchedules?.map((staffWithSchedule) => (
                <SelectItem
